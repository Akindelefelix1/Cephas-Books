        ...
        <Modal open title="Sales receipt" onClose={() => setSelectedSale(null)} footer={null}>
          <div className="receipt-card">
            <div className="receipt-card__heading">
              ...
              <small>{selectedSale.receiptNumber}</small>
              <small>{new Date(selectedSale.createdAt).toLocaleString()}</small>
            </div>
            {selectedSale.items.map((item) => (
              <div className="receipt-card__row" key={`${item.description}-${item.quantity}`}>
                ...
                <strong>{money(Number(item.lineTotal), selectedSale.currency)}</strong>
              </div>
            ))}
            <div className="receipt-card__total">
              <span>Total</span>
              <strong>{money(Number(selectedSale.total), selectedSale.currency)}</strong>
            </div>
            <div className="receipt-card__actions">
              ...
              <button className="button" onClick={() => printReceipt(selectedSale, true)}>
